...
In sintesi, si tratta di lavoro preparatorio pre-implementazione piuttosto che di un sistema funzionante: regole del firewall, segmentazione VLAN, stack di monitoraggio o qualsiasi altro codice/configurazione per il lab non sono ancora stati committati, e il diagramma è ancora una bozza da finalizzare in draw.io. Si legge come una base di conoscenza personale che un ingegnere costruisce prima di cablare una rete domestica con un'appliance firewall dedicata e un monitoraggio di sicurezza di base, attualmente nella fase di pianificazione e raccolta di riferimenti.
